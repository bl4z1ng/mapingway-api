...
namespace Mapingway.Presentation.Swagger.Documentation;
...
public class SwaggerControllerOrder<T>
{
    ...
    private SwaggerControllerOrder(IEnumerable<Type>? controllers)
    {
        ...
        // from the attribute and store it as controllerName -> sortOrder pair in the (case-insensitive) dictionary.
        _orders = new Dictionary<string, uint>(
            controllers!
                .Where(c => c.GetCustomAttributes<SwaggerControllerOrderAttribute>().Any())
                .Select(c => new
                {
                    Name = ResolveControllerName(c.Name), c.GetCustomAttribute<SwaggerControllerOrderAttribute>()!.Order
                })
    ...
    private uint Order(string? controller)
    {
        ...
        if (!_orders.TryGetValue(controller!, out var order))
            order = uint.MaxValue;
